...
function getRelevantSafetyModule(safetyType = 'traffic_safety') {
  ...
  return {
    title: module.title,
    bulletPoints: module.content.map(item => `• ${item}`).join('\n')
  };
}
...
**2. 행사 소개 및 의미**
"${eventName}"는 ${eventPurpose}를 통해 우리 아이들이 더욱 의미있는 학교생활을 경험할 수 있는 소중한 기회입니다. 이번 행사를 통해 학생들은 친구들과 협력하며 성장하고, 새로운 경험을 통해 꿈을 키워나갈 것입니다.
...
**4. 프로그램 안내**
${eventProgram}을 통해 학생들이 즐겁고 보람찬 시간을 보낼 수 있도록 세심하게 준비하였습니다. 각 프로그램은 아이들의 창의성과 협동심을 기를 수 있도록 구성되어 있습니다.

**5. 당부 말씀**
학부모님의 관심과 격려가 아이들에게 큰 힘이 됩니다. 행사 참여나 기타 문의사항이 있으시면 언제든 ${contactInfo}로 연락해주시기 바랍니다.
...
위 구조를 참고하여 자연스럽고 완성된 가정통신문을 작성해주세요. 모든 정보가 자연스럽게 통합되어야 하며, 딱딱하지 않고 따뜻한 느낌이 들도록 해주세요.`;
  ...
  generateGradePrompt(data) {
    const {
      ...
      studentStrengths = '성실하고 책임감이 강합니다',
...
다음과 같은 흐름으로 자연스럽고 개인적인 편지를 작성해주세요:

**1. 마음을 담은 인사**
${this.seasonalGreeting}

안녕하십니까? 오늘 ${studentName} 학생의 성적표를 전해드리면서, 한 학기 동안 함께하며 지켜본 ${studentName}의 소중한 성장 이야기를 나누고 싶어 이렇게 편지를 쓰게 되었습니다.

**2. 학생의 특별한 모습들**
${studentName}는 ${praiseExpressions}하며, 특히 ${studentStrengths}한 모습이 선생님의 마음을 따뜻하게 해주었습니다. ${academicProgress}한 점에서 ${studentName}만의 특별함을 발견할 수 있었습니다.

**3. 성장의 의미**
성적표는 숫자로 나타나지만, 진정한 배움은 점수로 측정할 수 없는 마음의 성장에 있다고 생각합니다. ${studentName}가 보여준 노력과 성실함, 그리고 친구들을 배려하는 따뜻한 마음이야말로 가장 소중한 성과라고 할 수 있습니다.

**4. 함께 키워나갈 부분**
앞으로 ${improvementAreas}하면 ${studentName}의 숨어있는 재능과 가능성이 더욱 빛날 것이라 확신합니다. 이는 단점이 아니라 ${studentName}가 더욱 성장할 수 있는 여지이자 기회입니다.

**5. 격려와 전망**
${encouragementMessage}. 학부모님의 사랑과 격려, 그리고 선생님의 관심이 함께한다면 ${studentName}는 분명히 자신만의 특별한 꿈을 이루어낼 것입니다.

**6. 감사 인사**
항상 ${studentName}의 교육에 깊은 관심을 가져주시고 학교 활동에 적극 협조해주시는 학부모님께 진심으로 감사드립니다.

${formatKoreanDate(this.currentDate)}
담임교사 올림

위 구조를 바탕으로 ${studentName} 학생에게 특화된 따뜻하고 개인적인 편지를 작성해주세요. 성적보다는 학생의 인격과 성장에 초점을 맞춘 격려의 메시지가 되도록 해주세요.`;
  }
...
다음과 같은 구조로 안전의 중요성을 강조하면서도 실행 가능한 지침을 제공해주세요:

**1. 정중한 인사**
${this.seasonalGreeting}

**2. 안전 교육의 중요성**
${backgroundInfo}. 우리 아이들의 소중한 생명과 안전을 지키기 위해서는 학교와 가정이 함께 노력해야 합니다.

**3. ${safetyModule.title}**
다음 사항들을 자녀와 함께 확인하고 실천해주시기 바랍니다:

${safetyModule.bulletPoints}

**4. 가정에서의 안전 교육**
${specificInstructions}. 특히 평상시 자녀와 안전에 대해 대화하시며, 위험 상황에 대한 대처 방법을 미리 알려주시면 큰 도움이 됩니다.

**5. 협조 당부**
아이들의 안전은 아무리 강조해도 지나치지 않습니다. 학부모님의 세심한 관심과 지속적인 안전 교육이 우리 아이들을 지키는 가장 확실한 방법입니다.

**6. 감사 인사**
항상 자녀 안전에 깊은 관심을 가져주시는 학부모님께 감사드리며, 궁금한 사항이 있으시면 언제든 연락주시기 바랍니다.

${formatKoreanDate(this.currentDate)}
○○학교장

위 내용을 바탕으로 ${safetyTopic}에 관한 실용적이고 설득력 있는 안전 교육 통신문을 작성해주세요. 단순한 나열이 아니라 학부모님들이 실제로 실천할 수 있는 구체적인 방법을 제시해주세요.`;
  }
...
다음과 같은 구조로 설문조사의 목적과 중요성을 설명하며 참여를 독려해주세요:

**1. 정중한 인사**  
${this.seasonalGreeting}

**2. 설문조사 목적 설명**
더 나은 교육환경을 만들기 위해 "${surveyTitle}"를 실시하고자 합니다. 이번 설문조사는 ${surveyPurpose}을 통해 우리 아이들에게 더욱 향상된 교육 서비스를 제공하기 위함입니다.
...
**4. 참여 방법 안내**
설문조사는 ${surveyMethod}로 진행되며, 익명으로 처리됩니다. 학부모님의 소중한 의견은 학교 교육 정책 수립과 프로그램 개선에 귀중한 자료로 활용될 것입니다.

**5. 개인정보 보호**
설문조사 과정에서 수집되는 모든 정보는 개인정보보호법에 따라 안전하게 관리되며, 연구 목적 이외에는 사용되지 않습니다.

**6. 협조 요청 및 감사**
바쁘신 중에도 우리 아이들의 교육을 위해 시간을 내어주시는 학부모님께 깊은 감사를 드립니다. 여러분의 참여가 더 나은 교육환경을 만드는 밑거름이 됩니다.

문의사항이 있으시면 언제든 학교로 연락주시기 바랍니다.

${formatKoreanDate(this.currentDate)}
○○학교장

위 구조를 바탕으로 "${surveyTitle}"에 대한 학부모님들의 적극적인 참여를 이끌어낼 수 있는 설득력 있는 통신문을 작성해주세요. 설문조사의 가치와 중요성을 자연스럽게 강조해주세요.`;
  }
...
다음과 같은 구조로 ${studentName}에게 특화된 개인적이고 구체적인 편지를 작성해주세요:

**1. 개인적인 인사**
${this.seasonalGreeting}

안녕하십니까? ${observationPeriod} 동안 ${studentName}와 함께 보낸 소중한 시간들을 되돌아보며, 이 아이만의 특별한 성장 이야기를 학부모님과 나누고 싶어 펜을 들었습니다.

**2. ${studentName}만의 특별한 순간들**
${studentName}가 보여준 ${specificBehaviors}는 정말 인상적이었습니다. 특히 기억에 남는 것은:
- 수업 시간에 보여준 집중력과 적극적인 참여 자세
- 어려운 문제를 만나도 포기하지 않고 끝까지 해결하려는 의지  
- 친구들을 도와주는 따뜻한 마음과 배려심
- 새로운 것을 배울 때 보이는 반짝이는 눈빛

**3. 눈에 띄는 성장과 발전**
${achievements}을 통해 ${studentName}의 발전을 확실히 느낄 수 있었습니다. 또한 ${socialInteraction}을 보며, 이 아이가 단순히 학업적으로만 성장하는 것이 아니라 한 명의 훌륭한 사람으로 자라나고 있음을 실감합니다.

**4. 함께 키워나가고 싶은 보물들**  
${studentName}가 ${recommendedSupport}하면 숨어있는 재능들이 더욱 빛날 것입니다. 이는 부족함이 아니라, 이 아이가 더 큰 가능성을 가지고 있다는 증거입니다.

**5. 가정에서의 격려 방법**
- ${studentName}의 작은 시도와 노력에 대해 구체적으로 인정해주세요
- 결과보다는 과정과 노력을 칭찬해주세요  
- 실수를 했을 때도 다시 시작할 수 있다는 용기를 주세요
- ${studentName}만의 속도와 방식을 존중해주세요

**6. 미래에 대한 기대**
앞으로 ${studentName}가 보여줄 ${futureGoals}가 정말 기대됩니다. 이 아이 안에는 무한한 가능성이 잠들어 있으며, 적절한 시기에 아름답게 꽃피울 것이라 확신합니다.

항상 ${studentName}의 성장을 함께 지켜봐 주시는 학부모님께 깊은 감사를 드립니다.

${formatKoreanDate(this.currentDate)}
담임교사 올림

위 구조를 바탕으로 ${studentName} 학생에게만 해당하는 구체적이고 개인적인 관찰 내용이 담긴 따뜻한 피드백을 작성해주세요. 이 아이만의 독특한 특성과 성장 모습이 생생하게 드러나도록 해주세요.`;
  }
}
